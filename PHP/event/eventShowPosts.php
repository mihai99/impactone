...
$res = mysqli_query($conn, "SELECT * FROM `events` WHERE id='" . $idPostat . "'");
if (mysqli_num_rows($res) > 0) {
    while($row = mysqli_fetch_assoc($res)) {
        $pozaPostat=$row["cover"];
...
$res = mysqli_query($conn, "SELECT * FROM `".$idPostat."_ev_posts` WHERE id='" . $postare . "'");
if (mysqli_num_rows($res) > 0) {
    while($row = mysqli_fetch_assoc($res)) {
        $poza=$row["photo"];
		$text=$row["text"];
...
echo'<img src="event'.$idPostat.'/'.$pozaPostat.'" width="100%" height="100%" style="border-top-left-radius: 20px;border-bottom-left-radius: 20px;">';
echo'</div>';
echo'	<div id="post" class="text1" style="">';
echo $text;
if($poza!="")echo "<img src='event".$idPostat. "/" .$poza. "' width='100%'>";
echo '</div>';
...
echo				$text;
if($poza!="") echo			'</div>';
if($poza!="") echo				"<img src='event".$idPostat. "/" .$poza. "' width='100%' style='max-height:400px;'>";
	echo	'</div>';
...
echo			'<form action="PHP/event/eventuploadComment.php" target="opt" onsubmit="setCookie(\'post\', '.$postare.');setTimeout(function () { window.location.reload(); }, 1)"  > ';
