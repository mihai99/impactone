...
$target_dir = "event".$_COOKIE["idviewevent"]."/";
...
$id++;
echo "---" . $id;
$sql="INSERT INTO `".$_COOKIE["idviewevent"]."_ev_photos` (`id`, `photo`) VALUES ('".$id."','".basename( $_FILES["fileToUpload"]["name"])."')";
echo "<br>" . $sql ."<br>";
if(mysqli_query($conn, $sql)) echo "ok2<br>"; 
}

$id=1;
$res = mysqli_query($conn, "SELECT * FROM `".$_COOKIE["idviewevent"]."_ev_posts`;") ;
$id=mysqli_num_rows($res);
$id++;
$sql="INSERT INTO `".$_COOKIE["idviewevent"]."_ev_posts` (`id`, `text`, `photo`) VALUES ('".$id."','".$txt."', '".basename( $_FILES["fileToUpload"]["name"])."')";
	if(mysqli_query($conn, $sql)) echo "<script>alert(3)</script><br>";

	$sql="CREATE TABLE ".$_COOKIE["idviewevent"]."_".$id."_ev_comments (
id INT ,
idpers VARCHAR(255) ,
comm VARCHAR(255)
)";
