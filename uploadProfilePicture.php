...
$result = mysqli_query($conn, "UPDATE `users` SET `profilePic`='".basename( $_FILES["fileToUpload"]["name"])."' WHERE id='".$_COOKIE["idloggedon"]."';") ;
if($result) echo "ok1<br>"; else echo mysql_error($conn); 
...
echo "<br>" . $sql ."<br>";
if(mysqli_query($conn, $sql)) echo "ok2<br>"; else echo mysql_error();
...
$sql="INSERT INTO `".$_COOKIE["idloggedon"]."_posts` (`id`, `text`, `photo`) VALUES ('".$id."','', '".basename( $_FILES["fileToUpload"]["name"])."')";
	if(mysqli_query($conn, $sql)) echo "3<br>";
